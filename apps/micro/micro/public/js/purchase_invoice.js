...
frappe.ui.form.on('Purchase Invoice', {
    onload: function(frm) {
        if(frm.doc.supplier){
            frappe.call({
                method: 'micro.micro.overrides.purchase_invoice.check_suplier_multiple_tds',
                args: {
    ...
    refresh: frm => {
        ...
        if (frm.doc.items) {
            if (frm.doc.items[0].purchase_receipt) {
                frappe.call({
                    method: 'micro.micro.methods.purchase_invoice.get_due_date_based_on_condition',
    ...
    before_save: frm => {
        ...
        frappe.call({
            method: 'micro.micro.overrides.purchase_invoice.get_item_wise_tax_rcm',
            args: {
    ...
    taxes_and_charges_overide: frm => {
            return frm.call({
              method: "micro.micro.overrides.purchase_invoice.get_taxes_and_charges_override",
              args: {
