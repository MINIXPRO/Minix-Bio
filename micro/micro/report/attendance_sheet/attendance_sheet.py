...
def execute(filters: Optional[Filters] = None) -> Tuple:
	filters.update({"summarized_view":0})
	filters = frappe._dict(filters or {})

	if not (filters.from_date and filters.to_date):
		frappe.throw(_("Please Select To Date and From Date"))
...
def get_columns_for_days(filters: Filters) -> List[Dict]:
	total_days = (getdate(filters.to_date) - getdate(filters.from_date)).days
	days = []

	months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
	for i in range(0,total_days+1):
		day = frappe.utils.add_to_date(getdate(filters.from_date), days=i,as_string=True)
		month = months[int(str(day).split("-")[1])-1]
		day = frappe.utils.add_to_date(getdate(filters.from_date), days=i,as_string=True).split("-")[2]
		label = "{} {}".format(cstr(day),cstr(month))
		days.append({"label": label, "fieldtype": "Data", "fieldname": day, "width": 90})
...
def get_attendance_status_for_summarized_view(
	...
	total_days = (filters.to_date - filters.from_date).days
